...
const SubTaskForm: React.FC<Props> = ({ onAdd }) => {
	...
	return (
		<form onSubmit={submitHandler} className={``}>
			<div
				className={`p-2 md:max-w-[99%] lg:max-w-[95%] flex items-center border-2 border-slate-300 rounded-md shadow-xl ${error.hasError
					? 'border-rose-300'
					: ''}`}
			>
				<label
				...
				>
					<FontAwesomeIcon icon={faPlusMinus} className={`max-w-[2.3rem] text-2xl`} />
				</label>
				<input
					type='text'
